...
export function ensureAndroidNexoraBridge(){
  ...
  (window as any).nexora={
    ...
    exportWholeMatchPack:()=>resolved({ok:false,error:'Export du pack disponible depuis la version PC.'}),
    saveMatchPresetSnapshot:()=>resolved({ok:false,error:'Sauvegarde du preset disponible depuis la version PC.'}),
    stopMatchScene:()=>resolved(true),
