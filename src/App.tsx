...
type Tab='dashboard'|'triggers'|'overlays'|'tts'|'actions'|'analytics'|'games'|'media'|'obs'|'gifts';
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    return()=>{offEvent();offStatus();offStats();offAction();offAnalytics();offObs();offGifts();};
  },[]);
  ...
  return <div className="app">
    ...
    <aside><div className="brand">NEXORA <b>LÉGEND</b></div><nav>{nav('dashboard','⌂ Tableau de bord')}{nav('triggers','⚡ Triggers')}{nav('overlays','▣ Overlays')}{nav('tts','🔊 TTS')}{nav('media','🎬 Médias')}{nav('games','🎮 Mini-jeux')}{nav('obs','◉ OBS')}{nav('gifts','🎁 Cadeaux')}{nav('actions','⌁ Actions')}{nav('analytics','▥ Analytics')}</nav></aside>
    <main>
      ...
      {activeTab==='obs'&&<section className="card"><div className="feedHead"><div><h2>OBS WebSocket</h2><p>Contrôle scènes, sources et audio directement depuis Nexora.</p></div><span className={obsConnected?'online':'offline'}>{obsConnected?'● OBS connecté':'● OBS hors ligne'}</span></div><div className="settingsGrid"><label>Adresse WebSocket<input value={settings.obsWsUrl||''} onChange={e=>setSettings(s=>({...s,obsWsUrl:e.target.value}))}/></label><label>Mot de passe<input type="password" value={settings.obsPassword||''} onChange={e=>setSettings(s=>({...s,obsPassword:e.target.value}))}/></label></div><div className="row"><button onClick={connectObs}>Connecter OBS</button><button onClick={disconnectObs}>Déconnecter</button><button onClick={refreshObsScenes}>Actualiser scènes</button></div>{obsError&&<div className="error">{obsError}</div>}<div className="settingsGrid"><label>Scène<select value={selectedScene} onChange={e=>setSelectedScene(e.target.value)}>{obsScenes.map(s=><option key={s}>{s}</option>)}</select></label><button onClick={()=>selectedScene&&window.nexora.obsAction(`scene:${selectedScene}`)}>Basculer sur la scène</button></div><div className="copyBox"><input value={obsManualAction} onChange={e=>setObsManualAction(e.target.value)} placeholder="mute:Micro ou source-off:Scene|Cam"/><button onClick={()=>window.nexora.obsAction(obsManualAction)}>Exécuter</button></div></section>}
      {activeTab==='gifts'&&<section className="card"><div className="feedHead"><div><h2>Catalogue cadeaux TikTok</h2><p>Le catalogue se remplit automatiquement pendant les lives.</p></div><span>{gifts.length} cadeau{gifts.length>1?'x':''}</span></div>{gifts.length===0?<div className="empty">Aucun cadeau détecté pour l'instant.</div>:<div className="giftGrid">{gifts.slice().sort((a,b)=>b.diamonds-a.diamonds).map(g=><div className="giftCard" key={g.id}>{g.image&&<img src={g.image} alt=""/>}<div><b>{g.name}</b><small>{g.diamonds} diamant{g.diamonds>1?'s':''}</small></div><button onClick={()=>navigator.clipboard.writeText(g.name)}>Copier nom</button></div>)}</div>}</section>}
      {activeTab==='actions'&&<section className="card"><h2>Actions système</h2><p>Teste les actions que tu pourras ensuite déclencher avec un cadeau, commentaire, follow, etc.</p><div className="copyBox"><input value={launchCommand} onChange={e=>setLaunchCommand(e.target.value)} placeholder="notepad.exe"/><button onClick={()=>window.nexora.launch(launchCommand)}>Lancer programme</button></div><div className="copyBox"><input value={hotkeyValue} onChange={e=>setHotkeyValue(e.target.value)} placeholder="ctrl+shift+f"/><button onClick={()=>window.nexora.hotkey(hotkeyValue)}>Tester raccourci</button></div><div className="copyBox"><input value={mouseValue} onChange={e=>setMouseValue(e.target.value)} placeholder="click:left ou move:500:300"/><button onClick={()=>window.nexora.mouse(mouseValue)}>Tester souris</button></div></section>}
      {activeTab==='games'&&<section className="card"><h2>Mini-jeux</h2><div className="gameCards"><div><b>🎡 Roue</b><p>Choix : {games.wheel.join(', ')||'aucun'}</p><div className="row"><input value={wheelInput} onChange={e=>setWheelInput(e.target.value)} placeholder="Nouveau choix"/><button onClick={()=>{if(!wheelInput.trim())return;saveGames({...games,wheel:[...games.wheel,wheelInput.trim()]});setWheelInput('');}}>Ajouter</button><button onClick={spinWheel}>Tourner</button></div></div><div><b>🎁 Giveaway</b><p>{games.giveaway.length} participant(s). Les viewers peuvent écrire !giveaway.</p><div className="row"><input value={giveawayInput} onChange={e=>setGiveawayInput(e.target.value)} placeholder="Ajouter un participant"/><button onClick={()=>{if(!giveawayInput.trim())return;saveGames({...games,giveaway:[...new Set([...games.giveaway,giveawayInput.trim()])]});setGiveawayInput('');}}>Ajouter</button><button onClick={pickGiveaway}>Tirer</button><button onClick={()=>saveGames({...games,giveaway:[]})}>Vider</button></div></div><div><b>📊 Sondage</b><input value={games.poll.question} onChange={e=>saveGames({...games,poll:{...games.poll,question:e.target.value}})} placeholder="Question"/><input value={games.poll.options.join(', ')} onChange={e=>saveGames({...games,poll:{...games.poll,options:e.target.value.split(',').map(v=>v.trim()).filter(Boolean),votes:{}}})} placeholder="Oui, Non"/><p>{Object.keys(games.poll.votes).length} vote(s)</p></div></div>{gameResult&&<div className="resultBox">{gameResult}</div>}</section>}
