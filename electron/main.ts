...
function runHotkey(value:string){
  const parts=value.toLowerCase().split('+').map(x=>x.trim()).filter(Boolean); if(!parts.length)return;
  const key=parts.pop()!; const mods=parts.map(m=>m==='ctrl'?'control':m==='win'?'command':m) as any[];
  try{robot.keyTap(key,mods);}catch{}
}
function runMouse(value:string){
  const [action,a,b]=value.split(':');
  try{
    if(action==='click') robot.mouseClick((a as any)||'left',false);
    else if(action==='double') robot.mouseClick((a as any)||'left',true);
    else if(action==='move') robot.moveMouse(Number(a)||0,Number(b)||0);
    else if(action==='scroll') robot.scrollMouse(0,Number(a)||0);
  }catch{}
}
function executeTrigger(t:Trigger,p:{type:string;user:string;detail:string;raw?:any}){
  const v=template(t.actionValue,p);
  if(t.action==='notify'){if(Notification.isSupported())new Notification({title:'Nexora Légend',body:v||`${p.user} · ${p.detail}`}).show();}
  else if(['sound','overlay','tts','media'].includes(t.action)){if(t.action==='overlay')broadcastOverlay({text:v||`${p.user} · ${p.detail}`,duration:5000});win?.webContents.send('trigger:action',{action:t.action,value:v,payload:p});}
  else if(t.action==='webhook'&&/^https?:\/\//i.test(v)){fetch(v,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(p)}).catch(()=>{});}
  else if(t.action==='launch'&&v){try{spawn(v,[],{detached:true,stdio:'ignore',shell:true}).unref();}catch{}}
  else if(t.action==='hotkey'&&v)runHotkey(v);
  else if(t.action==='mouse'&&v)runMouse(v);
  else if(t.action==='obs'&&v)void runObsAction(v);
}
function updateGiftCatalog(raw:any){
  const name=raw?.giftName||raw?.extendedGiftInfo?.name; if(!name)return;
  const diamonds=Number(raw?.diamondCount||raw?.extendedGiftInfo?.diamond_count||0); const image=raw?.giftPictureUrl||raw?.extendedGiftInfo?.icon?.url_list?.[0];
  const list=getGiftCatalog(); const idx=list.findIndex(g=>g.name===name); const item={id:String(raw?.giftId||name),name,diamonds,image}; if(idx>=0)list[idx]={...list[idx],...item}; else list.push(item); saveGiftCatalog(list); win?.webContents.send('gifts:update',list);
}
function pushEvent(type:string,user='',detail='',raw:any=null){
  const p={type,user,detail,raw,timestamp:Date.now()};win?.webContents.send('tiktok:event',p);
  analytics.events++;if(type==='comment')analytics.comments++;if(type==='like')analytics.likes+=Number(raw?.likeCount||1);if(type==='gift'){analytics.gifts+=Number(raw?.repeatCount||1);analytics.giftCoins+=Number(raw?.diamondCount||raw?.extendedGiftInfo?.diamond_count||0)*Number(raw?.repeatCount||1);updateGiftCatalog(raw);}if(type==='follow')analytics.follows++;if(type==='share')analytics.shares++;
  writeJson('analytics.json',analytics);win?.webContents.send('analytics:update',analytics);
  for(const t of activeTriggers())if(matchesTrigger(t,type,detail))executeTrigger(t,p);
}
...
ipcMain.handle('overlay:url',async()=>`http://127.0.0.1:${getSettings().overlayPort||18181}/overlay`);ipcMain.handle('overlay:test',async(_e,text:string)=>{broadcastOverlay({text,duration:5000});return true;});
