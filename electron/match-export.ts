...
type MatchPayload={left?:string;right?:string;scoreA?:number;scoreB?:number;name?:string;duration?:number;accent?:string;secondary?:string;introText?:string;outroText?:string;logoLeft?:string;logoRight?:string;packId?:string;__targetPath?:string};
...
function htmlForScene(scene:MatchScene,payload:MatchPayload){
  const accent=String(payload.accent||'#8b7cff');
  const secondary=String(payload.secondary||'#ff5f8f');
