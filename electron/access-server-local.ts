...
type Store = { founderPasswordHash:string; members:Member[]; audit:AuditEntry[] };
...
function loadStore():Store{try{return JSON.parse(fs.readFileSync(dataFile(),'utf8'));}catch{const s:Store={founderPasswordHash:bcrypt.hashSync(founderPassword,12),members:[],audit:[]};saveStore(s);return s;}}
...
export function startLocalAccessServer(){
  ...
  server=http.createServer(async(req,res)=>{
    ...
    if(req.method==='POST'&&url.pathname==='/founder/login'){try{const body=await readBody(req);if(!bcrypt.compareSync(String(body.password||''),store.founderPasswordHash))return json(res,401,{ok:false,error:'Identifiants invalides'});const token=jwt.sign({role:'founder',sub:'founder'},jwtSecret,{expiresIn:'15m'});audit(store,'founder-login','founder',undefined,req);return json(res,200,{ok:true,token,expiresInSeconds:900});}catch{return json(res,400,{ok:false,error:'Requête invalide'});}}
    if(req.method==='GET'&&url.pathname==='/members'){if(!founder(req))return json(res,401,{ok:false,error:'Non autorisé'});return json(res,200,{ok:true,members:store.members,audit:store.audit.slice(0,100)});}
    if(req.method==='POST'&&url.pathname==='/members'){if(!founder(req))return json(res,401,{ok:false,error:'Non autorisé'});try{const body=await readBody(req);const tiktok=normalizeTikTok(String(body.tiktok||''));if(!tiktok)return json(res,400,{ok:false,error:'@TikTok requis'});let m=store.members.find(x=>x.tiktok===tiktok);if(m){m.enabled=true;m.revokedAt=undefined;m.note=String(body.note||m.note||'');}else{m={id:crypto.randomUUID(),tiktok,enabled:true,createdAt:Date.now(),note:String(body.note||'')};store.members.push(m);}audit(store,'member-enabled','founder',tiktok,req);return json(res,200,{ok:true,member:m});}catch{return json(res,400,{ok:false,error:'Requête invalide'});}}
    if(req.method==='DELETE'&&url.pathname.startsWith('/members/')){if(!founder(req))return json(res,401,{ok:false,error:'Non autorisé'});const tiktok=normalizeTikTok(decodeURIComponent(url.pathname.slice('/members/'.length)));const m=store.members.find(x=>x.tiktok===tiktok);if(!m)return json(res,404,{ok:false,error:'Créateur introuvable'});m.enabled=false;m.revokedAt=Date.now();audit(store,'member-revoked','founder',tiktok,req);return json(res,200,{ok:true,member:m});}
